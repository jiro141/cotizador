...
export default function Buttons() {
    ...
    return (
        <div className={`button-container-wrapper ${isSliderActive ? 'slider' : 'grid'}`}>
            {isSliderActive && (
                <button className="slider-button slider-button-left" onClick={scrollLeft}>
                    <IoIosArrowBack size={24} color="#ff5722" />
                </button>
            )}
            <div className={`button-container ${isSliderActive ? 'button-container-slider' : 'button-container-grid'}`} ref={containerRef}>
                <button className={`button ${state === 'LandingPageClickThrough' ? 'active' : ''}`} onClick={() => handleButtonClick('LandingPageClickThrough')}>
                    <img src={img1} alt="Landing Page Click Through" />
                    <p className="button-text">Landing Page Click through</p>
                </button>
                <button className={`button ${state === 'LandingBasica' ? 'active' : ''}`} onClick={() => handleButtonClick('LandingBasica')}>
                    <img src={img1} alt="Web de Inicio" />
                    <p className="button-text">Web de Inicio (Landing básica)</p>
                </button>
                <button className={`button ${state === 'WebDeReservaciones' ? 'active' : ''}`} onClick={() => handleButtonClick('WebDeReservaciones')}>
                ...
                <button className={`button ${state === 'Blog' ? 'active' : ''}`} onClick={() => handleButtonClick('Blog')}>
            ...
            {isSliderActive && (
                <button className="slider-button slider-button-right" onClick={scrollRight}>
